...
- UAT (12): bảng Story × AC × kết quả; mỗi phát hiện có bằng chứng + mức Critical/Medium/Low; mục "đã hứa trong PRD mà thiếu" (soi kể cả khi test pass hết).
- Release (13): runbook (chạy sao, hỏng thường gặp xử sao, rollback); Go/No-Go đối chiếu KỲ VỌNG BAN ĐẦU (mục kỳ vọng trong README + success metric trong problem/) từng cái một: đạt / lệch-chấp-nhận / chưa — cổng đậm, verdict cuối chờ user ký.
- Operate (14): số đo thật so với success metric, bài học, ≥0 task đề xuất cho vòng kế (LEARN → quay lại 9).
