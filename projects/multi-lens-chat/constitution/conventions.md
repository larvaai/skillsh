# Quy ước chung — luật bất biến của project
...
Kết thúc một bước phải để lại trạng thái máy đọc được, không phải câu văn "đã xong". Chuỗi cổng: artifact sinh ra → `checkpoint` cấp phiếu `PASS`/`FAIL` → `progress` lật task `done` và mở khóa phụ thuộc. Người ký GO/NO-GO ở các cổng đậm (live slice GĐ8, release GĐ13). AI chuẩn bị đủ để ký, không tự ký thay.
...
Để sau này audit lại vì sao chọn hướng này. Từ GĐ6 trở đi (kiến trúc, stack, module) bắt buộc kèm `rationale` + `rejected_alternatives`.

## 8. YAML description

Trong `SKILL.md`, nếu `description:` chứa dấu hai chấm thì phải quote cả chuỗi, nếu không parser hỏng.
